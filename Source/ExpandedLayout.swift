...
public class ExpandedLayout: UICollectionViewFlowLayout {
    ...
    private var isTransitingToExpanded = false {
        didSet {
            ...
            didFinishExpandTransition = !isTransitingToExpanded
            invalidateLayout()
        }
    }
    ...
    public override func prepare() {
        ...
        guard !isTransitingToCollapsed else {
            collapseInvisibleSections()
            return
        }

        guard !isTransitingToExpanded else {
        ...
        if didFinishExpandTransition {
            setRealOffset()
            didFinishExpandTransition = false
        }
    }
    ...
    public override func prepareForTransition(from oldLayout: UICollectionViewLayout) {
        ...
        if oldLayout is CollapsedLayout {
            isTransitingToExpanded = true
        }
    }

    public override func finalizeLayoutTransition() {
        super.finalizeLayoutTransition()
        isTransitingToCollapsed = false
        isTransitingToExpanded = false
    }
    ...
    public override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        guard isTransitingToCollapsed || isTransitingToExpanded else {
            ...
            for elementAttributes in attributes {
                ...
                guard headersAttributes.indices.contains(elementAttributes.indexPath.section) else {
                    continue
                }
    ...
    public override func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint) -> CGPoint {
        ...
        for section in 0..<targetSection {
            ...
            targetOffset.y += sectionInset.top
            var sectionContentHeight: CGFloat = 0
            let numberOfItems = itemsAttributes[section].count
            for row in 0..<numberOfItems {
                let itemSize = itemsAttributes[section][row].frame.size
                ...
                if column == columnsCount - 1 || row == numberOfItems - 1 {
                    ...
                    if row < numberOfItems - 1, itemSize.height > 0 {
                        targetOffset.y += minimumLineSpacing
                    }
                }
            }
            targetOffset.y += sectionInset.bottom
        ...
        let emptySpace = collectionView.bounds.size.height - (max(contentHeight, collectionView.bounds.size.height) - targetOffset.y)
        if emptySpace > 0 {
    ...
    public func collapseInvisibleSections() {
        ...
        let previousItemsAttributes = itemsAttributes
        ...
        for section in 0..<numberOfSections {
            ...
            if visibleSections.contains(section) {
                ...
                if section < targetSection {
                    contentOffset.y -= sectionInset.top
                    contentOffset.y += minimumSectionSpacing
                }
            }
            ...
            for row in 0..<numberOfItems {
                ...
                if column == columnsCount - 1 || row == numberOfItems - 1 {
                    ...
                    if row < numberOfItems - 1, !visibleItemIndexPaths.contains(indexPath), section < targetSection {
                        contentOffset.y -= minimumLineSpacing
                    }
                    if !visibleItemIndexPaths.contains(indexPath), section == targetSection, hasVisibleItemsAfterCurrent {
                        contentOffset.y -= minimumLineSpacing
                    }
            ...
            if visibleSections.contains(section) {
                ...
                contentHeight += minimumSectionSpacing / 2
                if section < targetSection {
                    contentOffset.y -= sectionInset.bottom
                }
            }

            if sectionHeadersPinToVisibleBounds {
                let headerAttributes = headersAttributes[section]
                if section == targetSection,
                    contentHeight - contentOffset.y < collectionView.contentInset.top {
                    headerAttributes.frame.origin.y = min(contentHeight - headerAttributes.frame.size.height, contentOffset.y)
                    let originY = headerAttributes.frame.origin.y
                    for i in (0..<section).reversed() {
                        headersAttributes[i].frame.origin.y = originY - headersAttributes[i].frame.size.height * CGFloat(section - i)
                    }
                    ...
                    let originY = min(contentHeight - headerAttributes.frame.size.height, contentOffset.y)
                    for i in (0..<section).reversed() {
                        headersAttributes[i].frame.origin.y = originY - headersAttributes[i].frame.size.height * CGFloat(section - i)
                    }
                    headerAttributes.frame.origin.y = originY
    ...
    public func expandVisibleSections() {
        ...
        let visibleItemIndexPaths = determineVisibleIndexPaths()
        let visibleSections = Set<Int>(visibleItemIndexPaths.map { $0.section })
        ...
        if sectionHeadersPinToVisibleBounds {
            ...
            let visibleContentOffset = contentHeight - visibleFrameHeight
            for section in 0..<numberOfSections {
                if visibleSections.contains(section) {
                    if headersAttributes[section].frame.origin.y < visibleContentOffset {
                        if section <= targetSection, section < numberOfSections - 1, headersAttributes[section + 1].frame.origin.y > visibleContentOffset {
                            if section != targetSection {
...
private extension ExpandedLayoutPrivate {
    ...
    private func determineVisibleIndexPaths() -> [IndexPath] {
        ...
        let numberOfSections = dataSource.numberOfSections!(in: collectionView)
        for section in targetSection..<numberOfSections {
            ...
            visibleContentHeight += sectionInset.top
            ...
            for row in 0..<numberOfItems {
                ...
                if column == columnsCount - 1 || row == numberOfItems - 1 {
                    ...
                    if row < numberOfItems - 1 {
                        visibleContentHeight += minimumLineSpacing
                    }
                    guard visibleContentHeight < visibleFrameHeight else {
                        return visibleItems
                    }
            ...
            visibleContentHeight += sectionInset.bottom
        }

        for section in (0..<targetSection).reversed() {
            visibleContentHeight += sectionInset.bottom
            let numberOfItems = dataSource.collectionView(collectionView, numberOfItemsInSection: section)
            for row in 0..<numberOfItems {
                let indexPath = IndexPath(row: row, section: section)
    ...
    private func setRealOffset() {
        ...
        var contentOffset: CGFloat = offsetCorrection
        for section in 0..<targetSection {
            let headerHeigth = headersAttributes[section].frame.size.height
            contentOffset += headerHeigth
            contentOffset += sectionInset.top
            ...
            for row in 0..<numberOfItems {
                ...
                if column == columnsCount - 1 || row == numberOfItems - 1 {
                    ...
                    if row < numberOfItems - 1 {
                        contentOffset += minimumLineSpacing
                    }
                }
            }

            contentOffset += sectionInset.bottom
        }
        let maxContentOffset = max(contentHeight, collectionView.bounds.size.height) - collectionView.bounds.size.height
